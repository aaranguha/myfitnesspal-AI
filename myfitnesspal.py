...
def create_session(cookie_override=None):
    """Create a requests session using the cookie string from .env (or override)."""
    cookie_string = cookie_override or os.getenv("MFP_COOKIE")

    if not cookie_string or cookie_string == "PASTE_YOUR_COOKIE_HERE":
        msg = (
            "No MFP cookie found! Here's how to get it:\n"
            "  1. Open Chrome and go to: myfitnesspal.com/food/diary\n"
            "  2. Press F12 (or Cmd+Option+I) to open DevTools\n"
            "  3. Click the 'Network' tab, then reload the page (Cmd+R)\n"
            "  4. Click the first request in the list\n"
            "  5. Under 'Request Headers', find 'Cookie:'\n"
            "  6. Copy the ENTIRE value after 'Cookie: '\n"
            "  7. Paste it in your .env file as:\n"
            "     MFP_COOKIE=the_whole_cookie_string_here"
        )
        print(msg)
        raise RuntimeError("MFP cookie not configured")
    ...
    if resp.status_code in (301, 302, 303):
        ...
        if "login" in location or "account" in location:
            ...
            raise RuntimeError("MFP cookie expired")

    if resp.status_code != 200:
        print(f"Unexpected response ({resp.status_code}). Cookie may be expired.")
        raise RuntimeError(f"MFP auth failed (status {resp.status_code})")
